...
func generateSpecTypes(enumValues map[string][]string) (string, error) {
	...
	b.WriteString("package v2\n\n")
	b.WriteString("import \"encoding/json\"\n\n")

	// Enums (sorted by enum name; values sorted for determinism).
	...
	for _, name := range names {
		values := append([]string(nil), enumValues[name]...)
		slices.Sort(values)
...
func generateRuntimeTypes(enumValues map[string][]string) (string, error) {
	...
	// Keep stable copies of enums needed at runtime.
	if values, ok := enumValues["DatasetErrorKind"]; ok {
	...
	if values, ok := enumValues["ScopeKind"]; ok {
		values = append([]string(nil), values...)
		slices.Sort(values)
		values = slices.Compact(values)
